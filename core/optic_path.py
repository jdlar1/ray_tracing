...
class OpticalSystem:
    ...
    def load(self, image_name, output_size = None):
        ...
        print(self.ishape)
        self.transformed = np.zeros((*self.ishape))
    ...
    def add_space(self, d, n = 1):
        ...
        if self.A == [0, 0, 0]:
            self.A[0] = np.array([[1, 0],[d/n[0], 1]])
            self.A[1] = np.array([[1, 0],[d/n[1], 1]])
            self.A[2] = np.array([[1, 0],[d/n[2], 1]])
        else:
            self.A[0] = self.A[0].dot(np.array([[1, 0],[d/n[0], 1]]))
            self.A[1] = self.A[1].dot(np.array([[1, 0],[d/n[1], 1]]))
            self.A[2] = self.A[2].dot(np.array([[1, 0],[d/n[2], 1]]))

        if self.d0 == None:
            self.d0 = d 
            self.n0 = np.array(n)

    def add_single_len(self, R1, R2, nl):
        # Surfaces power
        nl_ = np.array(nl) 
        ...
        #Modify global A
        self.A[0] = self.A.dot(np.array([[a1[0],a2[0]],[a3[0],a4[0]]]))
        self.A[1] = self.A.dot(np.array([[a1[1],a2[1]],[a3[1],a4[1]]]))
        self.A[2] = self.A.dot(np.array([[a1[2],a2[2]],[a3[2],a4[2]]]))
